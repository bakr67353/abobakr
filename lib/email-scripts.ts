...
export async function createScript(script: { name: string; subject: string; body: string; user_id?: string }): Promise<EmailScript | null> {
  try {
    const response = await fetch("/api/scripts", {
      method: "POST",
...
export async function updateScript(id: string, updates: Partial<{ name: string; subject: string; body: string }>): Promise<boolean> {
  try {
